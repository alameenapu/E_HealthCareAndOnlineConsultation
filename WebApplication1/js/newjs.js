$(document).ready(function () {
    ...
    function doAjax(x) {
        ...
        $.ajax({
            ...
            method: "post",
            dataType: "json",
            data: JSON.stringify({ organ_name: x }),
            ...
            success: function (data) {
                ...
                function createDataTable(target, data) {
                    ...
                    $(target).DataTable({
                        ...
                        language: {
                            "emptyTable": "No Symptoms selected yet"
                        },
                        columnDefs: [{
                ...
                $('#symptomsTable').on("click", "tbody button", function () {
                    var id2 = $(this).closest("tr").find("td:nth-child(1)").text();
                    $.ajax({
                        url: "DiseaseService.asmx/GetDisease",
                        method: "post",
                        dataType: "json",
                        data: JSON.stringify({ symptoms_name: id2 }),
                        contentType: "application/json",
                        success: function (data) {
                            // $('#DiseaseTable').empty();
                            //creatDataTable('#DiseaseTable', null);
                            var sympList = 'GetDisease' ? JSON.parse(data.d) : data.d;
                            creatDataTable('#DiseaseTable', sympList);
                            function creatDataTable(target, data) {

                                $(target).DataTable({
                                    destroy: true,
                                    paging: false, searching: false, info: false, data: data,
                                    language: {
                                        "emptyTable": "No disease found"
                                    },
                                    columns: [
                                        {
                                            'data': 'Disease',
                ...
                $('#DiseaseTable').on("click", "tbody tr", function () {
                    var table = $('#DiseaseTable').DataTable();
                    var id1 = $(this).closest("tr").find("td:nth-child(1)").text();
                    document.location.href = id1 + ".aspx";
                });
